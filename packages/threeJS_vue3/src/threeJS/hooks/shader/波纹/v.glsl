...
void main(){
  ...
  float elevation=sin(modelPosition.x*10.+uTime)*sin(modelPosition.z*10.+uTime);
  elevation+=noiseScale*noise(vec2(modelPosition.xz*noiseFrequency)+uTime);
  uElevation=elevation;
  elevation*=waterScale;
